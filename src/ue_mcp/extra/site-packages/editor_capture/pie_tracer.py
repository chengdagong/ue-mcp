...
class PIETracer:
    ...
    def __init__(
        ...
        if screenshot_dir is None and capture_screenshots:
            # Default: create screenshots directory alongside output file
            output_dir = os.path.dirname(output_file)
            output_basename = os.path.splitext(os.path.basename(output_file))[0]
            self.screenshot_dir = os.path.join(output_dir, f"{output_basename}_screenshots")
        else:
        ...
        self._trace_data = {
            ...
            "metadata": {
                ...
                "screenshot_dir": self.screenshot_dir if capture_screenshots else None,
            }
        }
        ...
        # Ensure screenshot directory exists if capturing
        if capture_screenshots and self.screenshot_dir:
            _ensure_directory(self.screenshot_dir)
    ...
    def start(self):
        ...
        self._trace_data = {
            ...
            "metadata": {
                ...
                "screenshot_dir": self.screenshot_dir if self.capture_screenshots else None,
            }
        }
...
"""
    ...
    def _start_screenshot_capture(self):
        ...
        self._current_screenshot_queue = []
        sample_idx = self._sample_count - 1  # 0-indexed
        ...
        for actor_name, actor in self._actor_refs.items():
            try:
                ...
                for view_config in views_to_capture:
                    ...
                    safe_actor_name = actor_name.replace(" ", "_").replace("/", "_")
                    filename = os.path.join(
                        self.screenshot_dir,
                        f"sample_{sample_idx:04d}_{safe_actor_name}_{view_name}.png"
                    )
        ...
        if self._current_screenshot_queue:
            unreal.log(f"[INFO] Starting screenshot capture for sample #{sample_idx} ({len(self._current_screenshot_queue)} shots)")
    ...
    def _take_screenshot(self, shot_info):
        ...
        try:
            ...
            # Record screenshot path in trace data
            # Find the sample for this actor and add screenshot path
            if actor_name in self._trace_data["actors"]:
                samples = self._trace_data["actors"][actor_name]
                if sample_idx < len(samples):
                    if "screenshots" not in samples[sample_idx]:
                        samples[sample_idx]["screenshots"] = []
                    samples[sample_idx]["screenshots"].append(filename)

            unreal.log(f"[OK] Screenshot: {actor_name}/{view_config['name']} -> {os.path.basename(filename)}")
    ...
    def _on_executor_complete(self, executor):
        ...
        This is the on_before_complete callback that saves trace data
        ...
        unreal.log(f"[INFO] Tracer completing - saving trace data...")
        ...
        if self.capture_screenshots and self.screenshot_dir:
            self._result["screenshot_dir"] = self.screenshot_dir
        ...
        screenshot_info = f", screenshots: {self.screenshot_dir}" if self.capture_screenshots else ""
