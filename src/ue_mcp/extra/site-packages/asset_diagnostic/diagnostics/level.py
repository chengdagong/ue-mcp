...
class LevelDiagnostic(BaseDiagnostic):
    ...
    def _check_player_start(self, actors: List[unreal.Actor], result: DiagnosticResult):
        ...
        if not player_starts:
            result.add_issue(
                ...
                details=["Players will not have a spawn location"],
            ...
            result.add_issue(
                ...
                details=[f"PlayerStarts: {', '.join(ps_names)}"],
            )
    ...
    def _check_overlapping_actors(self, actors: List[Dict[str, Any]], result: DiagnosticResult):
        ...
        for i, a1 in enumerate(actors):
            ...
            for a2 in actors[i + 1 :]:
                ...
                min_separation = (
                    max(a1["x_extent"], a1["y_extent"]) + max(a2["x_extent"], a2["y_extent"])
                ) / 2
    ...
    def _detect_floating_objects(
        ...
        floating = [info.actor for info in actor_info_map.values() if not info.is_supported]
    ...
    def _check_floating_actors(self, actors: List[unreal.Actor], result: DiagnosticResult):
            ...
            if isinstance(actor, unreal.Character):
                ...
                if not skeletal_mesh:
                    result.add_issue(
                        IssueSeverity.ERROR,
                        "Configuration",
                        "Character actor missing SkeletalMeshComponent",
                        actor=actor_name,
                        details=[
                            f"Position: ({location.x:.0f}, {location.y:.0f}, {location.z:.0f})",
                            f"Class: {class_name}",
                            "Cannot determine bounds for floating detection without SkeletalMeshComponent",
                            "This Character will be excluded from floating object detection",
                        ],
                        suggestion="Add a SkeletalMeshComponent to the Character blueprint or instance",
                    )
                    continue
                ...
                if origin is None or extent is None:
                    result.add_issue(
                        IssueSeverity.ERROR,
                        "Configuration",
                        "Cannot retrieve bounds from Character's SkeletalMeshComponent",
                        actor=actor_name,
                        details=[
                            f"Position: ({location.x:.0f}, {location.y:.0f}, {location.z:.0f})",
                            f"Class: {class_name}",
                            f"SkeletalMeshComponent: {skeletal_mesh.get_name()}",
                            "Bounds retrieval failed - may indicate invalid mesh asset or configuration",
                            "This Character will be excluded from floating object detection",
                        ],
                        suggestion="Check SkeletalMesh asset validity and bounds configuration in the blueprint",
                    )
        ...
        # Build floating groups - find connected floating actors
        floating_set = set(floating_actors)
        floating_groups: List[List[unreal.Actor]] = []
        visited_floating = set()

        for actor in floating_actors:
            if actor in visited_floating:
                continue
            ...
            while queue:
                current = queue.pop(0)
                if current in visited_floating:
                    continue
                visited_floating.add(current)
                group.append(current)

                # Find overlapping floating actors
                if current in actor_info_map:
                    current_info = actor_info_map[current]
                    for other in floating_actors:
                        if other in visited_floating:
                            continue
                        if other in actor_info_map:
        ...
        # Report floating groups with detailed information
        for group in floating_groups:
            # Use the first actor as the primary actor for the issue
            primary_actor = group[0]
            primary_name = primary_actor.get_actor_label()
            primary_location = primary_actor.get_actor_location()
            primary_origin, primary_extent = get_actor_bounds(primary_actor)

            # Build details with position and size information
            details = []

            # Primary actor info
            details.append(f"--- Primary Actor: {primary_name} ---")
            details.append(
                f"Position: ({primary_location.x:.0f}, {primary_location.y:.0f}, {primary_location.z:.0f})"
            )
            details.append(f"Class: {primary_actor.get_class().get_name()}")
            if primary_origin and primary_extent:
                details.append(
                    f"Size (half-extents): ({primary_extent.x:.0f}, {primary_extent.y:.0f}, {primary_extent.z:.0f})"
                )
                details.append(
                    f"Bounds center: ({primary_origin.x:.0f}, {primary_origin.y:.0f}, {primary_origin.z:.0f})"
                )

            # Connected actors info (if group has more than one actor)
            if len(group) > 1:
                details.append("")
                details.append(f"--- Connected Floating Actors ({len(group) - 1} total) ---")
                for connected_actor in group[1:]:
                    c_name = connected_actor.get_actor_label()
                    c_location = connected_actor.get_actor_location()
                    c_origin, c_extent = get_actor_bounds(connected_actor)

                    connected_info = f"• {c_name}"
                    connected_info += (
                        f" | Position: ({c_location.x:.0f}, {c_location.y:.0f}, {c_location.z:.0f})"
                    )
                    if c_origin and c_extent:
                        connected_info += (
                            f" | Size: ({c_extent.x:.0f}, {c_extent.y:.0f}, {c_extent.z:.0f})"
                        )
                    details.append(connected_info)
            ...
            group_min_z = float("inf")
            group_max_z = float("-inf")

            for actor in group:
                origin, extent = get_actor_bounds(actor)
                if origin and extent:
                    group_min_x = min(group_min_x, origin.x - extent.x)
                    group_max_x = max(group_max_x, origin.x + extent.x)
                    group_min_y = min(group_min_y, origin.y - extent.y)
                    group_max_y = max(group_max_y, origin.y + extent.y)
                    group_min_z = min(group_min_z, origin.z - extent.z)
                    group_max_z = max(group_max_z, origin.z + extent.z)

            if group_min_z != float("inf"):
                details.append("")
                details.append("--- Floating Group Overall Info ---")
                details.append(
                    f"Group bounds: X[{group_min_x:.0f}, {group_max_x:.0f}], "
                    f"Y[{group_min_y:.0f}, {group_max_y:.0f}], "
                    f"Z[{group_min_z:.0f}, {group_max_z:.0f}]"
                )
                details.append(f"Group bottom Z: {group_min_z:.0f}")
            ...
            for g in ground_actors:
                ...
                dist = math.sqrt(
                    (primary_location.x - g_loc.x) ** 2
                    + (primary_location.y - g_loc.y) ** 2
                    + (primary_location.z - g_loc.z) ** 2
                )
            ...
            if nearest_ground:
                ...
                details.append("")
                details.append("--- Relationship to Ground ---")
                details.append(
                    f"Nearest ground: {nearest_ground.get_actor_label()} "
                    f"| Position: ({g_loc.x:.0f}, {g_loc.y:.0f}, {g_loc.z:.0f})"
                )
                if g_origin and g_extent:
                    details.append(
                        f"Ground size: ({g_extent.x:.0f}, {g_extent.y:.0f}, {g_extent.z:.0f})"
                    )
                    ground_top_z = g_origin.z + g_extent.z
                    if group_min_z != float("inf"):
                        gap = group_min_z - ground_top_z
                        details.append(f"Ground top Z: {ground_top_z:.0f}")
                        details.append(f"Vertical gap to ground: {gap:.0f} units")

            # Find nearest supported actor
            nearest_supported = None
            nearest_supported_dist = float("inf")
            if primary_actor in actor_info_map:
                actor_info = actor_info_map[primary_actor]
                for other, other_info in actor_info_map.items():
                    if other in floating_set or not other_info.is_supported:
                        continue
                    dist = math.sqrt(
                    ...
                    if dist < nearest_supported_dist:
                        nearest_supported_dist = dist
                        nearest_supported = other

            details.append("")
            details.append("--- Floating Status Explanation ---")
            if len(group) > 1:
                actor_names = [a.get_actor_label() for a in group]
                details.append(
                    f"The above {len(group)} actors ({', '.join(actor_names)}) are in contact and form a single unit"
                )
            else:
                details.append(f"Actor '{primary_name}' exists independently")

            if nearest_supported:
                details.append(
                    f"Nearest supported actor: {nearest_supported.get_actor_label()} "
                    f"(distance: {nearest_supported_dist:.0f} units)"
                )
                details.append("However, this group has NO contact with the supported actor")
            else:
                details.append("No supported actors nearby")

            details.append("This group has NO contact with the ground or any supported actors - it is floating")

            # Create issue message based on group size
            if len(group) > 1:
                issue_message = (
                    f"Floating group detected: {len(group)} connected actors are floating with no contact to ground or other actors"
                )
            else:
                issue_message = "Actor is floating with no contact to ground or other actors"
        ...
        for class_name, instances in blueprint_counts.items():
            if len(instances) > 1:
                ...
                result.add_issue(
                    IssueSeverity.INFO,
                    "Blueprint",
                    f"Multiple instances of blueprint class {class_name} found",
                    details=[
                        f"Instance count: {len(instances)}",
                        f"Instances: {', '.join(instances)}",
                    ],
                    suggestion="Review if multiple instances are intentional or if they should be deduplicated",
                )
    ...
    def _print_level_analysis(
        ...
        if tangible_actors:
            ...
            for i, actor in enumerate(sorted_actors[:20], 1):  # Top 20
                ...
                unreal.log(f"    Position: ({actor['x']:.0f}, {actor['y']:.0f}, {actor['z']:.0f})")
