...
def find_post_process_volumes(world):
    """
    Find all PostProcessVolume actors in the current world.

    Args:
        world: The loaded world/level

    Returns:
        list: List of PostProcessVolume actors
    """
    try:
        ...
        ppv_actors = [actor for actor in all_actors
                      if isinstance(actor, unreal.PostProcessVolume)]

        return ppv_actors
    except Exception as e:
        unreal.log_warning(f"[WARNING] Failed to find PostProcessVolumes: {e}")
        return []


def select_best_post_process_volume(ppv_list, target_location=None):
    """
    Select the most appropriate PostProcessVolume based on priority.

    Selection logic:
    1. Highest priority value wins
    2. If tie, prefer enabled volumes
    3. If tie, prefer unbound volumes (global)
    4. If tie and target_location provided, prefer closest

    Args:
        ppv_list: List of PostProcessVolume actors
        target_location: Optional target location for distance-based selection

    Returns:
        unreal.PostProcessVolume or None
    """
    if not ppv_list:
        return None

    # Filter to enabled volumes, fallback to all if none enabled
    enabled = [ppv for ppv in ppv_list if ppv.enabled]
    candidates = enabled if enabled else ppv_list

    # Sort by priority (highest first)
    candidates.sort(key=lambda ppv: ppv.priority, reverse=True)
    max_priority = candidates[0].priority

    # Filter to highest priority
    top_priority = [ppv for ppv in candidates if ppv.priority == max_priority]

    if len(top_priority) == 1:
        return top_priority[0]

    # Prefer unbound volumes
    unbound = [ppv for ppv in top_priority if ppv.unbound]
    if unbound:
        candidates = unbound if len(unbound) == 1 else unbound
    else:
        candidates = top_priority

    # Use distance if still multiple candidates
    if target_location and len(candidates) > 1:
        min_dist = float('inf')
        closest = candidates[0]
        for ppv in candidates:
            loc = ppv.get_actor_location()
            dist = math.sqrt(
                (loc.x - target_location.x) ** 2 +
                (loc.y - target_location.y) ** 2 +
                (loc.z - target_location.z) ** 2
            )
            if dist < min_dist:
                min_dist = dist
                closest = ppv
...
def extract_post_process_settings(ppv):
    """
    Extract PostProcessSettings from a PostProcessVolume.

    Args:
        ppv: PostProcessVolume actor

    Returns:
        tuple: (PostProcessSettings, blend_weight) or (None, None) if failed
    """
    try:
        settings = ppv.settings
        blend_weight = ppv.blend_weight

        if settings is None:
            unreal.log_warning("[WARNING] PostProcessVolume has no settings")
            return None, None

        return settings, blend_weight
    except Exception as e:
        unreal.log_warning(f"[WARNING] Failed to extract settings: {e}")
        return None, None
...
def capture_single_view(
    ...
    Args:
        ...
        use_scene_post_process: If True, copy post-processing from scene's
            PostProcessVolume instead of using manual exposure settings (default: False)
        scene_ppv_blend: Blend weight for scene PPV settings, 0.0 to 1.0 (default: 1.0)
        fallback_to_manual: If True, use manual exposure settings when no PPV found
            (default: True). If False, use UE5 defaults.
    ...
    should_use_manual = True

    if use_scene_post_process:
        # Try to find and use scene PostProcessVolume
        ppv_list = find_post_process_volumes(loaded_world)

        if ppv_list:
            selected_ppv = select_best_post_process_volume(ppv_list, camera_location)
            settings, blend_weight = extract_post_process_settings(selected_ppv)

            if settings:
                # Apply scene PPV settings
                capture_component.set_editor_property("post_process_settings", settings)
                capture_component.set_editor_property("post_process_blend_weight", scene_ppv_blend)

                unreal.log(f"  [PPV] Using: {selected_ppv.get_actor_label()}")
                unreal.log(f"  [PPV] Priority: {selected_ppv.priority}, Blend: {scene_ppv_blend:.2f}")
                should_use_manual = False
            else:
                unreal.log_warning("[WARNING] Failed to extract PPV settings")
                should_use_manual = fallback_to_manual
        else:
            unreal.log_warning("[WARNING] No PostProcessVolume found in scene")
            should_use_manual = fallback_to_manual
    ...
    if should_use_manual:
        ...
        # Tone Mapping (UE5 defaults)
        post_process_settings.set_editor_property("white_temp", 6500.0)
        post_process_settings.set_editor_property("white_tint", 0.0)

        # Bloom (UE5 defaults)
        post_process_settings.set_editor_property("bloom_intensity", 0.675)
        post_process_settings.set_editor_property("bloom_threshold", -1.0)

        # Color Grading - Neutral defaults
        post_process_settings.set_editor_property("color_gain_shadows", unreal.Vector4(1.0, 1.0, 1.0, 1.0))
        post_process_settings.set_editor_property("color_gain_midtones", unreal.Vector4(1.0, 1.0, 1.0, 1.0))
        post_process_settings.set_editor_property("color_gain_highlights", unreal.Vector4(1.0, 1.0, 1.0, 1.0))

        # Vignette (UE5 default)
        post_process_settings.set_editor_property("vignette_intensity", 0.4)
...
def take_orbital_screenshots(
    ...
    Args:
        ...
        auto_exposure_max: Maximum brightness for auto-exposure (default: 2.0)
        auto_exposure_bias: Exposure compensation in EV stops (default: 0.0)
        use_scene_post_process: Use PostProcessVolume settings from scene (default: False)
        scene_ppv_blend: Blend weight for scene PPV settings, 0.0-1.0 (default: 1.0)
        fallback_to_manual: If True, use manual settings when no PPV found (default: True)
        ...
        if enable_perspective_views:
            ...
            for yaw, pitch, view_name, display_name in PERSPECTIVE_VIEWS:
                ...
                saved_path = capture_single_view(
                    ...
                    use_scene_post_process=use_scene_post_process,
                    scene_ppv_blend=scene_ppv_blend,
