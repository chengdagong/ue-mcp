...
def capture_window(hwnd, crop_titlebar=True):
    ...
    """
    # Import PIL without catching - allows execute_code(checks=True) to detect and install Pillow
...
def capture_ue5_window(output_path, crop_titlebar=True):
    ...
    Raises:
        ImportError: If PIL/Pillow is not installed (allows auto-install mechanism to work)
    """
    # Don't catch ImportError - let it propagate for execute_code(checks=True) to handle
