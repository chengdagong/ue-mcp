...
class LevelDiagnostic(BaseDiagnostic):
    ...
    def _check_player_start(self, actors: List[unreal.Actor], result: DiagnosticResult):
        ...
        if not player_starts:
            result.add_issue(
                IssueSeverity.ERROR,
                "Gameplay",
                "Level has no PlayerStart actor",
                details=[
            ...
            result.add_issue(
                IssueSeverity.INFO,
                "Gameplay",
                f"Level contains {len(player_starts)} PlayerStart actors",
                details=[
        ...
        for ps in player_starts:
            ...
            if location.z < -1000:
                result.add_issue(
                    IssueSeverity.ERROR,
                    "Gameplay",
                    "PlayerStart positioned below valid spawn threshold",
                    actor=ps_name,
                    details=[
            ...
            for a2 in actors[i + 1 :]:
                ...
                if dist < overlap_threshold and a1["class"] == a2["class"]:
                    result.add_issue(
                        IssueSeverity.INFO,
                        "Collision",
                        f"Potential duplicate placement: 2 actors of class {a1['class']} overlapping",
                        details=[
                            ...
                            f"{_SEC_THRESHOLD} Overlap detection: distance < {overlap_threshold:.1f} units between same-class actors",
                            f"{_SEC_EXPECTED} Same-class actors typically separated by at least their bounding box width or intentionally stacked",
                            f"{_SEC_INFO} May indicate accidental duplicate placement, copy-paste error, or intentional clustering",
                        ],
                    )
    ...
    def _check_floating_actors(self, actors: List[unreal.Actor], result: DiagnosticResult):
        ...
        for cluster in floating_clusters:
            ...
            if len(group) > 1:
                issue_message = f"Unsupported actor group: {len(group)} connected actors floating above ground"
                ...
                issue_message = "Unsupported actor floating above ground"
        ...
        for actor in character_actors:
            ...
            if not is_horizontal:
                ...
                result.add_issue(
                    IssueSeverity.WARNING,
                    "Orientation",
                    "Character actor tilted beyond horizontal tolerance",
                    actor=actor_name,
                    details=details,
        ...
        for class_name, instances in blueprint_counts.items():
            if len(instances) > 1:
                ...
                result.add_issue(
                    IssueSeverity.INFO,
                    "Blueprint",
                    f"Blueprint class {class_name} has {len(instances)} instances in level",
                    details=[
