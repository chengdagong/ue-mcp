...
class _INPUT(ctypes.Structure):
    """Correct INPUT structure with anonymous union for SendInput API."""
...
def _send_key(vk_code, flags=0):
    ...
    inp.ki.wVk = vk_code
    inp.ki.wScan = _user32.MapVirtualKeyW(vk_code, 0)  # Get scan code for better compatibility
    inp.ki.dwFlags = flags
    inp.ki.time = 0
    inp.ki.dwExtraInfo = None
    result = _user32.SendInput(1, ctypes.byref(inp), ctypes.sizeof(_INPUT))
    if result != 1:
        error = ctypes.get_last_error()
        unreal.log_warning(f"[WARNING] SendInput returned {result}, last error: {error}, vk={hex(vk_code)}, flags={flags}")


def _press_key_combo(modifiers, key_vk):
    """Press a key combination (e.g., Ctrl+Shift+Alt+Number).

    Uses longer delays to ensure UE5 Editor properly registers the key combo.
    """
    ...
    # Small delay to ensure all modifiers are registered
    time.sleep(0.05)
...
def capture_window(hwnd, crop_titlebar=True):
    ...
    Returns:
        dict: {"success": True, "image": PIL Image} or {"success": False, "error": str}

    Raises:
        ImportError: If PIL/Pillow is not installed (allows auto-install mechanism to work)
    """
    # Import PIL without catching - allows execute_with_auto_install to detect and install Pillow
    ...
    try:
        ...
        if width <= 0 or height <= 0:
            error = f"Invalid window size ({width}x{height}) - window may be minimized"
            unreal.log_error(f"[ERROR] {error}")
            return {"success": False, "error": error}

        hwnd_dc = _user32.GetWindowDC(hwnd)
        if not hwnd_dc:
            error = "GetWindowDC failed - could not get device context"
            unreal.log_error(f"[ERROR] {error}")
            return {"success": False, "error": error}
        ...
        if not mfc_dc:
            ...
            error = "CreateCompatibleDC failed - could not create memory DC"
            unreal.log_error(f"[ERROR] {error}")
            return {"success": False, "error": error}
        ...
        if not bitmap:
            ...
            _user32.ReleaseDC(hwnd, hwnd_dc)
            error = f"CreateCompatibleBitmap failed for size {width}x{height}"
            unreal.log_error(f"[ERROR] {error}")
            return {"success": False, "error": error}
        ...
        if not result:
            ...
            bitblt_result = _gdi32.BitBlt(mfc_dc, 0, 0, width, height, hwnd_dc, 0, 0, _SRCCOPY)
            if not bitblt_result:
                unreal.log_warning("[WARNING] BitBlt also failed")
        ...
        getdib_result = _gdi32.GetDIBits(mfc_dc, bitmap, 0, height, buffer, ctypes.byref(bmi), _DIB_RGB_COLORS)
        ...
        if not getdib_result:
            error = "GetDIBits failed - could not extract bitmap data"
            unreal.log_error(f"[ERROR] {error}")
            return {"success": False, "error": error}
        ...
        error = f"Exception during capture: {e}"
        unreal.log_error(f"[ERROR] {error}")
        return {"success": False, "error": error}
...
def capture_ue5_window(output_path, crop_titlebar=True):
    ...
    Returns:
        dict: Result with 'success' bool and optional 'error' message
              e.g., {"success": True} or {"success": False, "error": "Could not find window"}

    Raises:
        ImportError: If PIL/Pillow is not installed (allows auto-install mechanism to work)
    """
    # Don't catch ImportError - let it propagate for execute_with_auto_install to handle
    _ensure_directory(output_path)

    hwnd = find_ue5_window()
    if not hwnd:
        error_msg = "Could not find UE5 Editor window"
        unreal.log_error(f"[ERROR] {error_msg}")
        return {"success": False, "error": error_msg}

    try:
        capture_result = capture_window(hwnd, crop_titlebar)
    except ImportError:
        # Re-raise ImportError to trigger auto-install
        raise
    except Exception as e:
        error_msg = f"Failed to capture window: {e}"
        unreal.log_error(f"[ERROR] {error_msg}")
        return {"success": False, "error": error_msg}
...
def open_asset_and_screenshot(asset_path, output_path, delay=3.0, tab_number=None, crop_titlebar=True):
    ...
    try:
        ...
        if capture_result.get("success"):
            ...
            unreal.log_error(f"[ERROR] Failed to capture window: {error}")
            result["error"] = error
