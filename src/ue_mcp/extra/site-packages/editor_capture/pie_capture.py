...
class PIECapturer:
    ...
    def start(self):
        ...
        if self.multi_angle and not pie_already_running:
            ...
            if self._camera_actor is None:
                unreal.log_warning("[WARNING] Failed to create camera, falling back to single-angle mode")
        ...
        if pie_already_running:
            ...
            if self.multi_angle and self._camera_actor is None:
                unreal.log_warning("[WARNING] Cannot create camera during PIE, using single-angle mode")
                self.multi_angle = False
    ...
    def _find_target_actor(self, pie_world):
        ...
        if target_lower in actors_by_class:
            ...
            if len(actors) > 1:
                matched_info = [{"label": a.get_actor_label(), "name": a.get_name(), "type": a.get_class().get_name()} for a in actors]
        ...
        if partial_name_matches:
            if len(partial_name_matches) > 1:
                matched_info = [{"label": a.get_actor_label(), "name": a.get_name(), "type": a.get_class().get_name()} for a in partial_name_matches]
        ...
        if partial_label_matches:
            if len(partial_label_matches) > 1:
                matched_info = [{"label": a.get_actor_label(), "name": a.get_name(), "type": a.get_class().get_name()} for a in partial_label_matches]
        ...
        if partial_class_matches:
            if len(partial_class_matches) > 1:
                matched_info = [{"label": a.get_actor_label(), "name": a.get_name(), "type": a.get_class().get_name()} for a in partial_class_matches]
    ...
    def _auto_complete(self):
        ...
        unreal.log(f"[OK] PIE capture completed - {self._screenshot_count} screenshots in {self._total_elapsed:.1f}s")
